...
<%      boolean islog=false;
        ...
        if (pjk.equals("1")) { tabname="rukuzz"; str="郑州检修车间备品库";}
        if (pjk.equals("2")) { tabname="rukuys"; str="月山检修车间备品库";}
        if (pjk.equals("3")) { tabname="rukuly"; str="洛阳检修车间备品库";}
        if (pjk.equals("4")) { tabname="rukuzl"; str="郑州轮轴车间备品库";}
        if (pjk.equals("5")) { tabname="rukud"; str="段材料库";}
...
%>
  ...
  <tr> <td><img width="694" height="68" src="Images/rukucx.gif" >  </td>
  </tr>
  <tr>
      <td height="25"><div>
     ...
     <input type="hidden" name="kc" value="4">
  ...
  <tr>
     <td align="center" height="25">查询的是:<%=str %>配件入库信息</td>
  </tr>
  <tr>
      <td height="25">
     <form name="formincx" method="post" action="pjgl/pjgl.jsp">请选择查询条件:
     <input type="radio" name="bhmc" value="1" <%if (bhmc.equals("1")) out.print("checked");%>> 入库日期
       ...
       if (Con != null)
        {
             try
             {
              String sql="";
              if (bhmc.equals("1")) { 
               sql ="select "+tabname+".id,"+tabname+".name,pjinfo.gg,pjinfo.dw,pjinfo.dj,"+tabname+".rq,"+tabname+".rukus,"+tabname+".czy from " +tabname+",pjinfo where "+tabname+".rq='"+pj+"' and "+tabname+".id=pjinfo.id"+" order by "+tabname+".rq DESC "; }
                else { if (bhmc.equals("2")) { 
                 sql ="select "+tabname+".id,"+tabname+".name,pjinfo.gg,pjinfo.dw,pjinfo.dj,"+tabname+".rq,"+tabname+".rukus,"+tabname+".czy from " +tabname+",pjinfo where "+tabname+".name like '"+pj+"%' and "+tabname+".id=pjinfo.id"+" order by "+tabname+".rq DESC "; }
                      else { if (bhmc.equals("3")) {
                        sql ="select "+tabname+".id,"+tabname+".name,pjinfo.gg,pjinfo.dw,pjinfo.dj,"+tabname+".rq,"+tabname+".rukus,"+tabname+".czy from " +tabname+",pjinfo where "+tabname+".id='"+pj+"' and "+tabname+".id=pjinfo.id"+" order by "+tabname+".rq DESC "; }
                           }
                     }
                 ...
                 CountR = Rs.getRow();
            		Rs.first();
            		    ...
            		    while (  !Rs.isAfterLast())	{     
                            ...
                            <td ><div align="center"><%=Rs.getString("rukus")%></div></td> 
