...
<html>
  ...
  <%
      ...
      if (request.getParameter("dtto")!=null)  dtto=new String(request.getParameter("dtto"));
         String StrPgzg = "1";
         String StrPgzj = "1";
       if (request.getParameter("pgcodezj")!=null) StrPgzj = request.getParameter("pgcodezj");
      if (request.getParameter("pgcodezg")!=null) StrPgzg = request.getParameter("pgcodezg");       
      ...
      { if ( dtfrom!=null  &&  dtto!=null && !dtto.equals("") && !dtfrom.equals("")) 
        ...
        { if (dtfrom!=null  &&  dtto!=null && !dtto.equals("") && !dtfrom.equals(""))  
         ...
         sql="select a.rq,a.money,c.name as kh,d.name as bkh,e.bz as banz,b.bz,b.kf1,b.gyx,b.jf1,a.ms from kpjl"+chej+"  as a ,cjkpbz as b,zhigong"+chej+" as c,zhigong"+chej+" as d ,banzu as e where a.bkhr='"+bkhr+"' and a.khr=c.id and a.bkhr=d.id and a.bz=e.code and a.kpbz=b.code order by rq desc";} 
          ...
          <td>
          <a href="cjgl/cjgl.jsp?page=2&subpg=5&dtfrom=<%=dtfrom%>&dtto=<%=dtto%>&cj=<%=chej%>&bzcode=<%=bzcode%>&pgcodezj=<%=StrPgzj%>&pgcodezg=1">首页</a>
           <%if (CurrPgzg>1){%>
            <a href="cjgl/cjgl.jsp?page=2&subpg=5&dtfrom=<%=dtfrom%>&dtto=<%=dtto%>&cj=<%=chej%>&bzcode=<%=bzcode%>&pgcodezj=<%=StrPgzj%>&pgcodezg=<%=CurrPgzg-1%>">上一页</a> 
            <% }  %>
            <%if (CurrPgzg<CountPgzg){%>
            <a href="cjgl/cjgl.jsp?page=2&subpg=5&dtfrom=<%=dtfrom%>&dtto=<%=dtto%>&cj=<%=chej%>&bzcode=<%=bzcode%>&pgcodezj=<%=StrPgzj%>&pgcodezg=<%=CurrPgzg+1%>">下一页</a> 
            <% } %>
            <a href="cjgl/cjgl.jsp?page=2&subpg=5&dtfrom=<%=dtfrom%>&dtto=<%=dtto%>&cj=<%=chej%>&bzcode=<%=bzcode%>&pgcodezj=<%=StrPgzj%>&pgcodezg=<%=CountPgzg%>">最后一页</a>
          </td>
          </tr>
        </table>
  <%    }
            		...
            		CurrPgzj = Integer.parseInt(StrPgzj);
            			...
            			%><table id="zgtab" width="98%" border="0" align="center" cellpadding="1" cellspacing="1" >
            			  ...
            			  <td></td></tr></table>
                            ...
                            <td  align="center"><%=Rszg.getString("money")%>
                            </td>
                           <%
					            Rszg.next();
					            j++;
          ...
          <td>
          <a href="cjgl/cjgl.jsp?page=2&subpg=5&dtfrom=<%=dtfrom%>&dtto=<%=dtto%>&cj=<%=chej%>&bzcode=<%=bzcode%>&pgcodezg=<%=StrPgzg%>&pgcodezj=1">首页</a>
           <%if (CurrPgzj>1){%>
            <a href="cjgl/cjgl.jsp?page=2&subpg=5&dtfrom=<%=dtfrom%>&dtto=<%=dtto%>&cj=<%=chej%>&bzcode=<%=bzcode%>&pgcodezg=<%=StrPgzg%>&pgcodezj=<%=CurrPgzj-1%>">上一页</a> 
            <% }  %>
            <%if (CurrPgzj<CountPgzj){%>
            <a href="cjgl/cjgl.jsp?page=2&subpg=5&dtfrom=<%=dtfrom%>&dtto=<%=dtto%>&cj=<%=chej%>&bzcode=<%=bzcode%>&pgcodezg=<%=StrPgzg%>&pgcodezj=<%=CurrPgzj+1%>">下一页</a> 
            <% } %>
            <a href="cjgl/cjgl.jsp?page=2&subpg=5&dtfrom=<%=dtfrom%>&dtto=<%=dtto%>&cj=<%=chej%>&bzcode=<%=bzcode%>&pgcodezg=<%=StrPgzg%>&pgcodezj=<%=CountPgzj%>">最后一页</a>
          </td>
          </tr>
        </table>
