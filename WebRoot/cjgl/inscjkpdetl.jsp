...
<html>
  ...
  <% 
          ...
          String khr=request.getParameter("khr");
          String bkhr=request.getParameter("bkhr");
		...
		if (maxId == null)
          ...
          sql="Insert into "+tabname+" (id,rq,khr,bkhr,bz,kpbz,ms) values ('"+maxId+"','"+rq+"','"+khr+"','"+bkhr+"','"+bz+"','"+kpbz+"','"+ms+"')";
          %><%=sql %><%
          ...
          if(rtn==0){%>
              ...
              <%   response.sendRedirect("inscjkp.jsp"); %>
          <%}else{%>
              <script language="javascript">
              alert("记录保存成功！");
              window.close();
