...
<%      boolean islog=false;
        ...
        tabname="dwtk"; str="[ 郑州北车辆段检修系统专项问题库 ]";
        
%>
   <table align="center" class="tableBorder_LR" width="871" border="0" cellpadding="-2" cellspacing="-2" bgcolor="#ffffff">
  <tr>
     <td> 
     <table align="center"  width="800" height="68" border="0" cellpadding="-2" cellspacing="-2" background="Images/zxwtk.gif">
     <tr>
  ...
  <tr>
     ...
     <tr>
         ...
         <%
            		...
            		try
            		{
            			...
            			%>
                            ...
                            <td width="15" align="center" class="tableBorder_Btm"><div >
                               <a href="wtk/editwtk.jsp?tabname=<%=tabname%>&id=<%=Rs.getString("num")%>" title="修改记录或问题销号">
          ...
          <td>
           <%if (CurrPg>1){%>
            <a href="wtk/wtk.jsp?page=3&pgcode=<%=CurrPg-1%>&lb=<%=lb%>&jj=<%=jj%>">上一页</a> 
            <%
          	}
          %>
            <%if (CurrPg<CountPg){%>
            <a href="wtk/wtk.jsp?page=3&pgcode=<%=CurrPg+1%>&lb=<%=lb%>&jj=<%=jj%>">下一页</a> 
            <%
          	}
          %>
            <a href="wtk/wtk.jsp?page=3&pgcode=<%=CountPg%>&lb=<%=lb%>&jj=<%=jj%>">最后一页</a>
            </td>
        </tr>
        <% 
				    }
     ...
     <tr>
            <% if((islog) && ((((String)session.getAttribute("deptment")).equals("admin"))
                            ...
                            <a href="wtk/wtk.jsp?jj=<%=jj%>&page=<%=1%>&lb=<%=lb%>" title="添加新问题到问题库" onClick="Javascript:window.open('inswtk.jsp?ID1=<%=tabname%>','','width=690,height=660')" >
                               添加问题</a>
                            </div>
                            </td>
                            <%} %>
